...
class AWPCP_UpgradeTaskController {
    ...
    public function run_task( $task_slug, $context ) {
        ...
        if ( is_null( $task ) ) {
            throw new AWPCP_Exception( sprintf( 'No task was found with identifier: %s.', $task_slug ) );
        }

        $task_handler = $this->task_handler_factory->get_task_handler( $task['handler'] );

        if ( is_null( $task_handler ) ) {
            throw new AWPCP_Exception( sprintf( "The handler for task '%s' couldn't be instantiated.", $task_slug ) );
        }
