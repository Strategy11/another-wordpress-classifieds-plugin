...
<p><?php _ex('You are about to pay for the following items. Please review the order and choose a payment method.', 'checkout step', 'another-wordpress-classifieds-plugin') ?></p>

<form class="awpcp-checkout-form" method="post">

    <h3><?php _ex('Payment Terms', 'checkout step', 'another-wordpress-classifieds-plugin'); ?></h3>

    <?php echo $this->render_account_balance_for_transaction( $transaction ); ?>

    <?php echo $this->render_transaction_items($transaction); ?>

    <h3><?php _ex('Payment Method', 'checkout step', 'another-wordpress-classifieds-plugin'); ?></h3>
