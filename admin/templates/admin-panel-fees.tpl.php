    ...
    if(awpcp_get_option( 'freepay' ) !='1' ){
        $payment_tab_link = sprintf( '<a href="%s">%s</a>', admin_url('admin.php?page=awpcp-admin-settings&g=payment-settings'), 'Payment page' );
        $notice = __('Currently your classifieds are in Free mode. Fee plans are not available or used during free mode.<br/><br/> To change this, visit the <payment-tab-link> and enable Charge Listing Fee setting.', 'another-wordpress-classifieds-plugin' );
        $notice = str_replace( '<payment-tab-link>', '<strong>' . $payment_tab_link . '</strong>', $notice );
        echo $notice;
