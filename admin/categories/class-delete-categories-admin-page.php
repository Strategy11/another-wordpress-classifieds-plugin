...
class AWPCP_Delete_Categories_Admin_Page {
    ...
    public function try_to_delete_categories() {
        ...
        } else if ( $result['categories_deleted'] == 0 ) {
            awpcp_flash( __( 'There was an error trying to delete the selected categories.', 'another-wordpress-classifieds-plugin' ), 'error' );
        } else {
    ...
    private function delete_categories( $selected_categories, $target_category, $should_move_listings ) {
        ...
        foreach ( $selected_categories as $category_id ) {
            try {
                ...
                $categories_deleted = $categories_deleted + 1;
            } catch ( AWPCP_Exception $e ) {
                $categories_not_deleted = $categories_not_deleted + 1;
                continue;
            }
        }
