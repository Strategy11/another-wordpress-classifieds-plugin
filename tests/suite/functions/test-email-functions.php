...
class AWPCP_TestEmailFunctions extends AWPCP_UnitTestCase {
    ...
    public function test_ad_enabled_user_email_body_is_escaped_for_plain_text() {
        ...
        wp_update_post(
            array(
                'ID'         => $listing->ID,
                'post_title' => 'Testanzeige "Quoted"',
            )
        );
        update_post_meta( $listing->ID, '_awpcp_contact_name', 'Jane "Doe"' );

        $body = $this->prepare_ad_enabled_user_email_body( get_post( $listing->ID ) );

        $this->assertStringContainsString( 'Hello Jane "Doe",', $body );
        $this->assertStringContainsString( 'Your Ad "Testanzeige "Quoted"" was recently approved', $body );
        $this->assertSame( false, strpos( $body, '&quot;' ) );
    }
    ...
    public function test_ad_enabled_user_email_footer_has_line_break_between_site_name_and_url() {
        $listing       = $this->create_test_listing_with_key( $this->key );
        $previous_name = get_option( 'blogname' );

        update_option( 'blogname', 'Dackel.de' );

        $body = $this->prepare_ad_enabled_user_email_body( $listing );

        update_option( 'blogname', $previous_name );

        $this->assertStringContainsString( 'Dackel.de' . PHP_EOL . home_url(), $body );
    }
     ...
     * @return string
     */
    private function prepare_ad_enabled_user_email_body( $listing ) {
        ...
        $email->prepare(
            AWPCP_DIR . '/frontend/templates/email-ad-enabled-user.tpl.php',
            compact( 'listing', 'listing_title', 'contact_name' )
        );
